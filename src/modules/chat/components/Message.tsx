...
export const Message = ({
  ...
  return (
    <div
      ...
      role="group"
      className={cn(
        "message-container flex items-end gap-2",
    ...
    >
      ...
      {status === "error" && onUndoSend && (
        <p
          role="button"
          aria-label="Undo Send"
          className="text-xs text-red-500 cursor-pointer hover:underline hover:font-medium"
          onClick={onUndoSend}
        >
          Undo Send
        </p>
      ...
      {status === "error" && onResend && (
        <p
          role="button"
          aria-label="Resend"
          className="text-xs text-blue-500 cursor-pointer hover:underline hover:font-medium"
          onClick={onResend}
        >
          Resend
        </p>
...
function MessageStatus({ status }: Pick<MessageProps, "status">) {
  ...
  return (
    <>
      ...
      {status === "success" && (
        <SuccessIcon
          className={clsx(baseClassName, "status-success text-green-500")}
        />
      )}
      {status === "error" && (
        <ErrorIcon
          className={clsx(baseClassName, "status-error text-red-500")}
        />
      )}
    </>
  );
